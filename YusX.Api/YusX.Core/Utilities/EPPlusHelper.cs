...
namespace YusX.Core.Utilities
{
    ...
    public class EPPlusHelper
    {
        ...
        /// <param name="ignoreTemplate">忽略不导出的列(如果设置了exportColumns,ignoreColumns不会生效)</param>
            ...
            Func<string, ExcelWorksheet, ExcelRange, int, int, string> readValue = null)
        {
            ...
            using (ExcelPackage package = new ExcelPackage(file))
            {
                if (package.Workbook.Worksheets.Count == 0 ||
                    package.Workbook.Worksheets.First().Dimension.End.Row <= 1)
                {
                    return responseContent.Error("未导入数据");
                }

                var cellOptions = GetExportColumnInfo(
                    typeof(T).Name,
                    temlate: false,
                    filterKeyValue: false,
                    columns: exportColumns?.GetExpressionToArray());
                //设置忽略的列
                if (exportColumns != null)
                {
                    cellOptions = cellOptions.Where(x => exportColumns.GetExpressionToArray()
                            .Select(s => s.ToLower())
                ...
                {
                    cellOptions = cellOptions.Where(x => !ignoreTemplate.Select(s => s.ToLower())
                            .Contains(x.ColumnName.ToLower()))
                        .ToList();
                }


                var sheetFirst = package.Workbook.Worksheets.First();
                ...
                var propertyInfos = typeof(T).GetProperties()
                    .Where(f => cellOptions.Select(s => s.ColumnName).Contains(f.Name))
                    .ToArray();
                ...
                for (int m = sheet.Dimension.Start.Row + 1, n = sheet.Dimension.End.Row; m <= n; m++)
                {
                    ...
                    for (int j = sheet.Dimension.Start.Column, k = sheet.Dimension.End.Column; j <= k; j++)
                    {
                        ...
                        if (!string.IsNullOrEmpty(options.DictNo) && !string.IsNullOrEmpty(value))
                        {
                            if (options.DictItems == null)
                            {
                                return responseContent.Error($"[{options.ColumnCNName}]字段数字典编号[{options.DictNo}]缺失,请检查字典配置");
                            }
                            string key = options.DictItems.Where(x => x.Value == value)
                                  .Select(s => s.Key)
                                  .FirstOrDefault();
                            if (key == null)//&& options.Requierd
                            {
                                //小于20个字典项，直接提示所有可选value
                                string values = options.DictItems.Count < 20 ? (string.Join(',', options.DictItems.Select(s => s.Value))) : options.ColumnCNName;
        ...
        public static string Export(DataTable table, List<CellOptions> cellOptions, string savePath, string fileName)
        {
            ...
            var dicNoKeys = cellOptions
                 .Where(x => !string.IsNullOrEmpty(x.DictNo) && x.DictItems != null && x.DictItems.Keys.Count > 0)
                 .Select(x => new { x.DictNo, x.ColumnName }).Distinct().ToList();
        ...
        {
            return Export<T>(null, exportColumns, ignoreColumns, savePath, fileName, template: true);
        }
        ...
        public static string Export<T>(List<T> list, IEnumerable<string> exportColumns, IEnumerable<string> ignoreColumns, string savePath, string fileName, bool template = false)
        {
            ...
            var dicNoKeys = cellOptions
                 .Where(x => !string.IsNullOrEmpty(x.DictNo) && x.DictItems != null && x.DictItems.Keys.Count > 0)
                 .Select(x => new { x.DictNo, x.ColumnName, x.SearchType, x.EditType }).Distinct().ToList();
        ...
        private static List<CellOptions> GetExportColumnInfo(string tableName, bool temlate = false, bool filterKeyValue = true, string[] columns = null)
        {
            ...
            var query = DbProvider.DbContext.Set<Sys_TableColumn>().Where(x => x.Name.ToLower() == tableName.ToLower());
            if (columns != null && columns.Length > 0)
            {
                query = query.Where(x => columns.Contains(x.Field));
            }
            else
            {
                query = query.Where(x => x.IsShow);
            }
            var cellOptions = query.Select(c => new CellOptions()
            {
                ColumnName = c.Field,
                ColumnCNName = c.Name,
                DictNo = c.DataSource,
            ...
            var dicNos = cellOptions.Where(x => !string.IsNullOrEmpty(x.DictNo)).Select(c => c.DictNo);
            ...
            foreach (string dicNo in dicNos.Distinct())
            {
                ...
                var dictionaryLists = dictionaries
                    .Where(x => x.DictNo == dicNo && x.DictionaryItems != null)
                    .Select(s => s.DictionaryItems)
                    .FirstOrDefault();
                ...
                foreach (var item in dictionaryLists)
                {
                    ...
                    if (filterKeyValue && item.Name == item.Value) continue;
                    if (keyValues.ContainsKey(item.Value)) continue;
                    keyValues.Add(item.Value, item.Name);
                }

                foreach (CellOptions options in cellOptions.Where(x => x.DictNo == dicNo))
                {
                    options.DictItems = keyValues;
                }
            }
            return cellOptions;
        }
    ...
    public class CellOptions
    {
        ...
        /// 相关字典子项
        /// </summary>
        public Dictionary<string, string> DictItems { get; set; }
    }
}
