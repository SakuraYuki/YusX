...
namespace YusX.Core.Providers
{
        ...
        where TEntity : BaseEntity
    {
        ...
        public RepositoryBase(YusXContext dbContext)
        {
            DefaultDbContext = dbContext ?? throw new ArgumentNullException("dbContext未实例化。");
        }

        /// <summary>
        /// 默认数据库上下文
        /// </summary>
        private YusXContext DefaultDbContext { get; set; }

        private YusXContext EFContext
        {
            get
            {
                DbProvider.GetDbContextConnection<TEntity>(DefaultDbContext);
        ...
        /// 数据库上下文
        /// </summary>
        public virtual YusXContext DbContext => DefaultDbContext;

        private DbSet<TEntity> DBSet => EFContext.Set<TEntity>();

        public ISqlDapper DapperContext => DbProvider.GetSqlDapper<TEntity>();
        ...
        public virtual List<TEntity> Find<Source>(IEnumerable<Source> sources, Func<Source, Expression<Func<TEntity, bool>>> predicate)
            where Source : class
        {
            return FindAsIQueryable(sources, predicate).ToList();
        }

        public virtual List<TResult> Find<Source, TResult>(
        ...
        /// <returns></returns>
        public virtual IQueryable<TEntity> FindAsIQueryable<Source>(
            IEnumerable<Source> sources,
            Func<Source, Expression<Func<TEntity, bool>>> predicate)
            where Source : class
        {
        ...
        public IQueryable<TEntity> FindAsIQueryable(
            Expression<Func<TEntity, bool>> predicate,
            Expression<Func<TEntity, Dictionary<object, QueryOrderBy>>> orderBy = null)
        {
            if (orderBy != null)
        ...
        /// <returns></returns>
        public virtual IQueryable<TFind> IQueryablePage<TFind>(
            int pageIndex,
            int pagesize,
            out int rowcount,
        ...
        /// <returns></returns>
        public virtual IQueryable<TEntity> IQueryablePage(
            ...
            bool returnRowCount = true)
        {
            ...
            return queryable.GetIQueryableOrderBy(orderBy)
                .Skip((pageIndex - 1) * pagesize)
                .Take(pagesize);
        }

        public virtual List<TResult> QueryByPage<TResult>(
            int pageIndex,
            int pagesize,
            out int rowcount,
            Expression<Func<TEntity, bool>> predicate,
            Expression<Func<TEntity, Dictionary<object, QueryOrderBy>>> orderBy,
            Expression<Func<TEntity, TResult>> selectorResult,
            bool returnRowCount = true)
        {
            return IQueryablePage(pageIndex, pagesize, out rowcount, predicate, orderBy, returnRowCount).Select(selectorResult).ToList();
        }

        public List<TEntity> QueryByPage(
            int pageIndex,
            int pagesize,
            out int rowcount,
            Expression<Func<TEntity, bool>> predicate,
            ...
            bool returnRowCount = true)
        {
            return IQueryablePage(pageIndex, pagesize, out rowcount, predicate, orderBy).ToList();
        }
        ...
        public virtual List<TResult> QueryByPage<TResult>(
            ...
            Expression<Func<TEntity, bool>> predicate,
            Expression<Func<TEntity, Dictionary<object, QueryOrderBy>>> orderBy,
            Expression<Func<TEntity, TResult>> selectorResult = null)
        {
            return IQueryablePage(pageIndex, pagesize, out int rowcount, predicate, orderBy).Select(selectorResult).ToList();
        }
        ...
        {
            return UpdateRange(new List<TSource>() { entity }, properties, saveChanges);
        }
        ...
        {
            return UpdateRange(
                entities: new List<TSource>() { entity },
        ...
        public virtual int UpdateRange<TSource>(
            ...
            bool saveChanges = false) where TSource : class
        {
            ...
            bool saveChange = false) where Detail : class
        {
            ...
            if (updateDetail)
            {
                ...
                if (detail != null)
                {
                    ...
                    message = UpdateDetail(obj as List<Detail>, key.Name, key.GetValue(entity), updateDetailFields, delNotExist);
                }
            }
